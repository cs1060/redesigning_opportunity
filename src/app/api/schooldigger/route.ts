...
const DEFAULT_APP_ID = 'redesigning-opportunity';
// Use the API key from the environment variable
const DEFAULT_API_KEY = process.env.NEXT_PUBLIC_SCHOOLDIGGER_API_KEY || '4a2120e17f61231455f92d5ae62057d7';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // The SchoolDigger API expects 'appKey' not 'apiKey'
    ...
    if (!data || !data.schoolList || !Array.isArray(data.schoolList)) {
      console.error('Invalid or empty response from SchoolDigger API');
      return NextResponse.json({
        count: 3,
        schools: generateMockSchools(zipCode),
        isMockData: true,
        error: 'Invalid or empty response from SchoolDigger API'
      });
    }

    console.log(`Found ${data.schoolList.length} schools from SchoolDigger API`);

    // Map SchoolDigger data to our application's format
    const formattedSchools = mapSchoolDiggerToAppFormat(data.schoolList);
